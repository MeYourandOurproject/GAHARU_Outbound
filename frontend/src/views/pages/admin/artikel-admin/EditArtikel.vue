<template>
  <div class="container-fluid">
    <div class="container text-start">
      <h3 class="mt-4 mb-4 fw-bold">Update Artikel</h3>
...
const fetchArtikelBySlug = async () => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/artikels/slug/${route.params.slug}`,
      {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/artikels/admin/${form.value.id}`,
      {
        method: "PUT",
        body: formData,
        headers: { Authorization: `Bearer ${token}` },
