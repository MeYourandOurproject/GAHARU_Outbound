<template>
  <div class="container-fluid">
    <div class="container text-start">
      <h3 class="mt-4 mb-4">EDIT PRICE LIST</h3>
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <!-- SUBMIT -->
        <button type="submit" class="btn btn-success">Update Price List</button>
      </form>
    </div>
...
const fetchPriceList = async () => {
  ...
  const response = await fetch(`${API_BASE_URL}/api/price-lists/slug/${slug}`, {
    method: "GET",
  ...
  // parse facilities JSON → textarea
  try {
    const facilitiesArray = JSON.parse(data.facilities);
    facilitiesText.value = facilitiesArray.join("\n");
  } catch {
...
const handleSubmit = async () => {
  ...
  const response = await fetch(`${API_BASE_URL}/api/price-lists/admin/${id}`, {
    method: "PUT",
    body: formData,
    headers: { Authorization: `Bearer ${token}` },
  });
  ...
  if (response.ok) {
    toastMessage.value = "Price List berhasil diupdate";
